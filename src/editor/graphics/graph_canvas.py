"""Behavior-graph canvas, node/edge/port items and specs (shared graphics primitive).

Lowered out of the panel-level ``graph_workspace`` module in ER6.0.  The class
and constant bodies are unchanged from their original home; only the module they
live in moved.  Both ``pattern_workspace`` (which embeds ``GraphCanvas``) and
``inspector_panel`` (which reads ``GRAPH_NODE_PROPERTY_SPECS``) now depend *down*
on this shared graphics layer instead of reaching sideways into a sibling panel,
so the author-document semantics and Qt behaviour are identical.  ``graph_workspace``
survives only as a logic-less re-export shim for existing importers.
"""
